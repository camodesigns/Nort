...
void UMultiplayerSessionsSubsystem::CreateSession(int32 MaxPublicConnections, const FString& MatchType)
{
	...
	const auto CreateSessionDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(
	...
	const bool bWasSuccessfulRequest = SessionInterface->CreateSession(*LocalPlayer->GetPreferredUniqueNetId(),
...
void UMultiplayerSessionsSubsystem::DestroySession()
{
	...
	const auto DestroySessionDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(
...
void UMultiplayerSessionsSubsystem::FindSessions(int32 MaxSearchResults)
{
	...
	const auto FindSessionsDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(
	...
	const ULocalPlayer* LocalPlayer = GetWorld()->GetFirstLocalPlayerFromController();
	const bool bWasSuccessfulRequest = SessionInterface->FindSessions(*LocalPlayer->GetPreferredUniqueNetId(),
	                                                                  LastSessionSearchRequest.ToSharedRef());
...
void UMultiplayerSessionsSubsystem::JoinSession(const FOnlineSessionSearchResult& DesiredSession)
{
	...
	const auto JoinSessionDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(
	...
	const ULocalPlayer* LocalPlayer = GetWorld()->GetFirstLocalPlayerFromController();
	const bool bWasSuccessfulRequest = SessionInterface->JoinSession(*LocalPlayer->GetPreferredUniqueNetId(),
	                                                                 NAME_GameSession, DesiredSession);
...
void UMultiplayerSessionsSubsystem::InitializeDelegates()
{
	if (SessionInterface)
	{
		const auto StartSessionDelegate = FOnStartSessionCompleteDelegate::CreateUObject(
